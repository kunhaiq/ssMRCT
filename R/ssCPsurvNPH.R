...
#' @param lambda0a vector rate parameter of the piece-wise exponential distribution on or before the breakpoint in the control group for all regions.
#' @param lambda0b vector rate parameter of the piece-wise exponential distribution after the breakpoint in the control group for all regions.
#' @param lambda1a vector rate parameter of the piece-wise exponential distribution on or before the breakpoint in the treatment group for all regions.
#' @param lambda1b vector rate parameter of the piece-wise exponential distribution before the breakpoint in the treatment group for all regions.
...
#' @param shape0 vector of the shape parameter of the weibull distribution in the control group for all regions (shape=1 reduces to exponential)
#' @param scale0 vector of the scale parameter of the weibull distribution in the control group for all regions (scale=1/rate)
#' @param shape1 vector of the shape parameter of the weibull distribution in the treatment group for all regions (shape=1 reduces to exponential)
#' @param scale1 vector of the scale  parameter of the weibull distribution in the treatment group for all regions (scale=1/rate)
#' @param PW logical; if TURE (default), survival time follow piece-wise exponential distribution. Otherwise weibull distribution
#' @param unimin vector of the lower limits of the uniform distribution.
#' @param unimax vector of the upper limits of the uniform distribution.
...
#' @param eta scalar of truncated time in RMST.
#'
#' @details
...
#'
#' Assume the survival endpoint follows either piece-wise exponential distribution or Weibull distribution.
#' The piece-wise exponential distribution with \code{lambda0a} or \code{lambda0b}=\eqn{\lambda_k^{(r)}}, \code{lambda1a} or \code{lambda1b}=\eqn{\gamma_k^{(r)}} and \code{breakpoint}=\eqn{\psi}
#' has hazard function of \eqn{\lambda_k^{(r)}I(0<t\le \psi )+\gamma_k^{(r)}I(t> \psi)} where \eqn{\psi} is the change point.
#' 1) \eqn{\Omega_r=\ell^{-1}\sigma_1^{2(r)}+\sigma_1^{2(r)}},
#' 2) \eqn{\delta} and \eqn{\tau} are derived based on the naive approach as the mean and standard deviation of the regional RMST between group difference.
#'
#'
#' @returns A list containing the following components:
#' \describe{
#'   \item{\code{Dr}}{regional effect}
#'   \item{\code{sigma2.0r}}{asymptotic vairance of RMST estimate of region r for the control group }
#'   \item{\code{sigma2.1r}}{asymptotic vairance of RMST estimate of region r for the treatment group }
...
#' @examples
#'
#' ssCPsurvNPH(alpha=0.025, beta=0.2,lambda0a=c(0.07,0.07,0.07,0.07), 
#'     lambda0b=c(0.03,0.04,0.05,0.06),lambda1a=c(0.02,0.03,0.04,0.05), 
#'     lambda1b=c(0.03,0.04,0.05,0.06),breakpoint=10, unimin=rep(0,4), 
#'     unimax=rep(80*3,4), pi=0.5, k=1, target=1, f=rep(1/4,4), eta=80)
#'
#' @export
#'
ssCPsurvNPH<-function(alpha, beta, lambda0a, lambda0b, lambda1a, lambda1b, breakpoint,
                      shape0,scale0,shape1,scale1, PW=T,
                      unimin, unimax, pi, k, target, f, eta){
  ...
  if(PW==T){  ### PW
    for (r in 1:length(f)){
      ...
      Omegars[r]<-k^(-1)*sigma2.1r[r]+sigma2.0r[r]
    }
  ...
  if(abs(sum(f)-1) > 10^(-10)){return(list("error: regional fractions do not sum up to 1"))}
    ...
    cp<-out$value/(1-beta)
    return(list("Dr"=delta.r, "sigma2.1r"=sigma2.1r,"sigma2.0r"=sigma2.0r,"n0"=n0, "n1"=n1, "n"=n0+n1, "CP"=round(cp,5)*100))
  }
  else{
