...
#' @param tau_delta_r The ratio of the standard deviation vs mean of the Gaussian prior of the regional treatment effect.
#' @param delta The mean of the Gaussian prior of the regional treatment effect.
#' @param sigma0r The standard deviation of response in the treatment group. Specified for region(s) of interest, either a scalar or vector of length equals to the total number of region(s) of interest.
#' @param sigma1r The standard deviation of response in the control group. Specified for all region(s), either a scalar or vector of length equals to the total number of region(s) of interest.
#' @param pi The threshold ratio in Japan's criterion I (conditional version). Defaults to 0.5.
#' @param k The randomization ratio between the treatment group and control group. Defaults to 1.
#' @param fr The regional fraction for the region(s) of interest, either a scalar or vector.
#'
#'
#' @details
...
#'
#' The sample size for the region of interest corresponding to the lower bound is calculated based on the below equation,
#' \deqn{
#' \frac{h_r}{h_r+1}=\frac{\tau^2}{2\delta^2}(z_{1-\alpha}+z_{1-\beta})^2, \quad n_1^{(r)}=kn_0^{(r)}
#' } 
#' where \eqn{h_r=\tau^2/\sigma^{2(r)}, \quad \sigma^{2(r)}=\Omega_r/(n_0f_r), \quad \Omega_r=k^{-1}\sigma_1^{2(r)}+\sigma_1^{2(r)}}
#' Then \eqn{n^{(r)} = n_0^{(r)}+n_1^{(r)}}. Additionally, both of
...
#'   \item{\code{n0r}}{The regional sample size of the control group. (n.a. is returned when the condition for positive sample size is not met)}
#'   \item{\code{n1r}}{The regional sample size of the treatment group. (n.a. is returned when the condition for positive sample size is not met)}
#'   \item{\code{nr}}{The regional sample size.(n.a. is returned when the condition for positive sample size is not met)}
#'
#'
#' }
#'
#' @examples
#'
#' CPlbd(alpha=0.05, beta=0.2, tau_delta_r=0.4, pi=0.5, delta=0.25, sigma0r=1,
#'        sigma1r=1, k=1, fr=c(0.1,0.5))
#'
#' @export
#'
CPlbd<-function(alpha, beta, tau_delta_r, delta, sigma0r,sigma1r,fr,pi,k){  #tau_delta_r is the ratio of tau divided by delta
  ...
  return(list("CPlbd"=round(cpmin,3)*100, "n0r"=n0r,"n1r"=n1r,"nr"=nr))
}
